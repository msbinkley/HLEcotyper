#!/usr/bin/env Rscript
# =============================================================================
# 06_visualize.R  --  QC + cohort high-confidence landscape
...
if (file.exists(qc_path)) {
  ...
  # Kraken2 unclassified rate (reads not passing configured thresholds, or no DB hit)
  if ("pct_unclassified" %in% names(qc)) op(print(
    qc %>% ggplot(aes(as.numeric(pct_unclassified))) +
      ...
      labs(title = "Kraken2 unclassified rate per sample",
           subtitle = "fraction of read pairs left unclassified by Kraken2",
